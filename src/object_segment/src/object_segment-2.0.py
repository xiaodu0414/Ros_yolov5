...
def callback_receive_img(data):
    try:
        ...
        q.put((data.width,data.height,data.header.frame_id,captureimg))
        if q.qsize() > 1:
            q.get() 
        else:
          time.sleep(0.001)
...
def sub_receive_img():
    ...
    # rospy.Subscriber("/object_location/object_detection",ObjectDetect,callback_receive_img)    
